...
class GGL_PSOD_Modified(GGL_PSOD):
    def run(self):
        for iter in range(self.max_iter):
            # Dynamiczna i nieliniowa aktualizacja parametrów wg modyfikacji A
            w = 0.9 - 0.5 * (iter / self.max_iter)**2 # paraboliczny spadek bezwładności
            c1 = 2.5 - (iter / self.max_iter) * (2.5 - 0.5) # poznawczy - niezależność na początku (c1=2.5)
            c2 = 0.5 + (iter / self.max_iter) * (2.5 - 0.5) # przyciąganie - gwałtowne zbieganie na końcu (c2=2.5)
            ...
            for i in range(self.ps):
                ...
                # Mutacja z rozkładem Cauchy'ego (Modyfikacja B)
                for d in range(self.dim):
                    if np.random.rand() < self.pm:
